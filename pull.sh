...
curl --silent 'https://en.wikipedia.org/w/index.php?title=Transformers&limit=5000&action=history' | perl -pse 's/^.*?oldid=(\d+).*$/\1/gm' | grep -v -E '[<":> \s]' | grep -E '[[:digit:]]' | uniq | sort -n | while read VERSION; do
